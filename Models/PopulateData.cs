...
public class PopulateData
{
    ...
    {
        using (var context = new D2RItemsContext(serviceProvider.GetRequiredService<DbContextOptions<D2RItemsContext>>()))
        {
            ...
            {
                using StreamReader file = File.OpenText(Path.Combine(webHostEnvironment.WebRootPath, "data", "weapons.json"));
            ...
            {
                using (StreamReader file = File.OpenText(Path.Combine(webHostEnvironment.WebRootPath, "data", "armor.json")))
                using (JsonTextReader reader = new JsonTextReader(file))
                {
